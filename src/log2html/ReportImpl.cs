...
public class ReportImpl
{
    ...
    private readonly ConcurrentBag<ReportEntry> _reportEntries = [];
    ...
    public void Log(ReportLevel level, string message, Exception ex = null, params IReportEntryVisitor[] reportEntryVisitors) => Log(ReportEntryFactory.Create(level, message, ex), reportEntryVisitors);
    ...
    public void AddLevelsSummary()
    {
        var levelsSummary = _reportEntries
            ...
            .ToArray();
        _reportFile.AddSummaryAndFinish(JsonConvert.SerializeObject(levelsSummary));
    }

    public void Open()
    {
        ProcessStartInfo psi = new(_reportFile.ReportFilePath) { UseShellExecute = true };
        Process.Start(psi);
    }
}