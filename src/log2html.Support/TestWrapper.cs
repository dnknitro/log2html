...
{
    public static void RunTest(Action testAction) => RunTest(new TestWrapperContext(testAction)).GetAwaiter().GetResult();

    public static void RunTest(IWebDriver webDriver, Action testAction) => RunTest(new TestWrapperContext(testAction) { WebDriver = webDriver }).GetAwaiter().GetResult();
